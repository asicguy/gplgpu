...
`include "../../stim/includes/clks_params.h"
  ...
  U0 
    (
     ...
     .dac_csyncn     (),
     .dac_sog        (),
     .bt_clko        (bt_clko),	  // Clock input from the BT868.
...
`include "../../stim/tasks/rw_tasks64.v"
  ...
  task wait_for_mc_a;
    begin
      rd(MEM_RD, rbase_a+FLOW,1);
      while (test_reg[1]) rd(MEM_RD, rbase_a+FLOW,1);
      #10000 rd(MEM_RD, rbase_a+FLOW,1);
...
///////////////////////////////////////////////////////////////////////
//
// This function converts a real (Double precition float) to bits.
// shortreal is not supported by most simulators at this time.
// Our design uses only shortreals.
// This function converts the real to 64 bits and then converts it to 32 bits.
// If there is an overflow + or - inf.
// If there is an underflow + or - zero.
//
function [31:0] f2h;
	input real	data_in;
	...
	begin
		h64 = $realtobits(data_in);
		if(h64[62:52] < 897) f2h = {h64[63],31'h0};
		else if(h64[62:52] > 1152) f2h = {h64[63], 31'h7fffffff};
		else begin
			h32[31] = h64[63];
			h32[30:23] = h64[62:52] - 896;
			h32[22:0]  = h64[51:29];
			f2h = h32;
		end
	end
...
///////////////////////////////////////////////////////////////////////
//
// This function converts a 32 bit value to a real (Double precition float).
// Shortreal is not supported by most simulators at this time.
// Our design uses only shortreals.
// This function converts 32 bits to the real.
// If there is an overflow + or - inf.
// If there is an underflow + or - zero.
//
function real h2f;
	...
	begin
		h64[63]    = data_in[31];
		h64[62:52] = data_in[30:23] + 896;
		h64[51:0]  = {data_in[22:0], 29'h0};
		h2f = $bitstoreal(h64);
	end
endfunction
