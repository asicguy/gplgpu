...
begin
 ...
 fail_count1= 0; //initialize the fail count
  ...
  $fdisplay(results, "\n*******************************************");
  $fdisplay(results, "            YUV_16_444 TEST START               ");
  $fdisplay(results, "*******************************************\n");
 

 
 REV2_STIM.open_file("hb_test_result/yuv_16_444.res"); //open up the dump result file
...
//****SET PACKING MODE FOR LINEAR 0 TO YUV_444 at 16BBP(565)
 ...
 mov_dw (MEM_WR, rbase_w+MW0_CTRL,   32'h24b8_0040, 4'h0, 1);  // MW0_CTRL(16 PACKING , 444)
...
//READ DATA FROM MEMORY
 ...
 for (i=0; i<=j; i=i+1)
  ...
  yuv_compare (write_mem_table[2*i], YUV,     first0_pixel, second0_pixel);
  yuv_compare (write_mem_table[(2*i)+1], YUV, first1_pixel, second1_pixel);
...
/***********************************/
  ...
  $fdisplay(results, "\n*******************************************");
  $fdisplay(results, "*******************************************");
  $fdisplay(results, "            YUV_16_444 TEST END               ");
  $fdisplay(results, "*******************************************");
  $fdisplay(results, "*******************************************\n");
 

 
 REV2_STIM.close_file("hb_test_result/yuv_16_444.res"); //close the dump file.
